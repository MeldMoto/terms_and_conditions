...
Wszystkie wyrażenia i słowa zapisywane za pomocą wielkiej litery (np. Aplikacja, Warsztat, Klient itd.) należy rozumieć zgodnie z [Regulaminem dla Warsztatów](https://meldmoto.github.io/terms_and_conditions/regulamin-warsztat).
...
Administrator pozyskuje informacje o danych osobowych Warsztatu w następujący sposób:
* a) poprzez dokonanie rejestracji Konta Warsztatu w Aplikacji;
* b) poprzez wprowadzenie opinii w Aplikacji;
* c) poprzez dobrowolnie wprowadzone informacje w formularzu kontaktowym lub w wiadomości e-mail;
* d) poprzez przesłanie reklamacji, wniosku, zapytania lub pisma o innym charakterze;
* e) poprzez dobrowolnie wprowadzone informacje w wiadomości e-mail przesłanej w związku z chęcią nawiązania współpracy;
* f) poprzez zamieszczenie przez Warsztat zdjęcia w profilu Warsztatu w Aplikacji;
* g) poprzez pliki cookies, piksele lub podobne technologie internetowe.

Informuję, że cel i zakres danych przetwarzanych przez Administratora wynika ze zgody Warsztatu lub przepisów prawa oraz w wybranych przypadkach jest doprecyzowywany w wyniku działań podejmowanych przez te osoby na stronie lub w ramach innych kanałów komunikacji.
...
| Zamieszczenie opinii przez Warsztat | art. 6 ust. 1 lit. a RODO | Nawiązanie dialogu z użytkownikiem badanie opinii Warsztatu. |
| Obsługa Konta Warsztatu | art. 6 ust. 1 lit. a RODO | Zawarcie i realizacja Umowy Usług Elektronicznych (Konto Warsztatu). |
